...
(deftest graph
  (with-graph {:layer (bal/make (tokyo/make {:path "/tmp/jiraph-test-tokyo-layer" :create true}) (raf/make))}
    (truncate! :layer)
    ...
    (testing "incoming keeps track of incoming edges"
      ...
      (is (= #{4} (incoming :layer 1)))
      (is (assoc-node! :layer 4 {:edges {2 {:foo 1} 3 {:bar 2}}}))
      (is (= #{}  (incoming :layer 1)))
